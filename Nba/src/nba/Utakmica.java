package nba;

import java.util.Date;

public class Utakmica {

	// potrebno jos napraviti rezultat prve ekipe i rezultat druge ekipe, za sada je napravljeno da samo jedan broj mogu unijeti
	private Date datumPocetka;
	private String domacaMomcad;
	private String gostujucaMomcad;
	private String nazivDvorane;
	private int rezultat;
	
	public Date getDatumPocetka() {
		return datumPocetka;
	}
	public void setDatumPocetka(Date datumPocetka) {
		this.datumPocetka = datumPocetka;
	}
	public String getDomacaMomcad() {
		return domacaMomcad;
	}
	public void setDomacaMomcad(String domacaMomcad) {
		this.domacaMomcad = domacaMomcad;
	}
	public String getGostujucaMomcad() {
		return gostujucaMomcad;
	}
	public void setGostujucaMomcad(String gostujucaMomcad) {
		this.gostujucaMomcad = gostujucaMomcad;
	}
	public String getNazivDvorane() {
		return nazivDvorane;
	}
	public void setNazivDvorane(String nazivDvorane) {
		this.nazivDvorane = nazivDvorane;
	}
	public int getRezultat() {
		return rezultat;
	}
	public void setRezultat(int rezultat) {
		this.rezultat = rezultat;
	}
	
	
	
}
